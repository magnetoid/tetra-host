...
- Keep the core clean and simple (rule `tetra-clean-simple-core`, ADR 0007): `app/main.py` is wiring only (middleware, plugin loading, request context) and cross-cutting logic lives in `app/services`; everything else is a plugin. Never grow the core to fit a feature — grow by adding a plugin. Design plugin-first and prefer the simplest thing that satisfies the charter (thin handlers, small single-purpose services, explicit typed boundaries) over premature abstraction, cleverness, or god-modules. New capability = new plugin, not a bigger core.
